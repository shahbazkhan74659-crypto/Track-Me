...
First confirmed the viewport meta tag wasn't actually missing — Next.js 16's App Router auto-injects `<meta name="viewport" content="width=device-width, initial-scale=1">` even though `app/layout.tsx` never declares one (verified directly against the rendered HTML), so no framework-level gap existed. The real work was fixing fixed-pixel layout that only worked at desktop widths: `StatCards.tsx`'s hard-coded 3-column grid (no stacking on narrow screens), `CalendarCard.tsx`'s fixed cell height/card padding/month-label width, two non-wrapping header flex rows (`AttendanceApp.tsx`, `SalarySetupControl.tsx`) that could overflow, and `.chevron-btn`'s 32×32px touch target (under the ~44px recommended minimum). `DateEntryModal`/`SalarySetupControl`'s modals, `LoginGate`, and `ProfileBadge` were already responsive-safe (`width + maxWidth:"100%"` pattern) and needed no structural change — only added `maxHeight:"90vh"; overflowY:"auto"` to both modals so they can't be clipped on short viewports (e.g. a phone keyboard covering half the screen).

Fixed via one new `@media (max-width: 640px)` block in `app/globals.css`, extending the exact pattern already established for `:hover` (Phase 10's decision: move anything inline styles can't express into a real CSS class) to media queries as well — inline styles can't express `@media` either. New classes: `.app-shell` (page padding), `.stat-cards-grid` (3-column → 1-column stack, reduced card padding), `.stat-value`/`.stat-value.net` (numeric font sizes), `.calendar-card` (card padding), `.calendar-cell` (day-cell height), `.month-label` (nav label width/font-size), plus a mobile-only bump on the existing `.chevron-btn`. No CSS framework, no JS-based viewport detection, no component converted from Server to Client — see `DECISIONS.md`.

**Status: Complete.** Verified 2026-09-05: `npm run build`/`npm run lint` both passed clean; grepping `components/StatCards.tsx` and `components/CalendarCard.tsx` for the removed inline properties (`fontSize: 28`/`30`, `height: 60`, `padding: 24`, `minWidth: 150`) confirmed none were left duplicated after moving them into CSS classes. **Browser-based visual verification at actual phone widths was not performed this session** — Chrome browser automation wasn't connected — so the owner should check the live app on a real phone or a narrow/resized browser window (or DevTools' device toolbar) before treating the mobile layout as final, same caveat as Phases 10–14 whenever Chrome automation was unavailable.

**Follow-up (2026-09-05, same day):** the owner asked for the calendar to appear before the stat cards specifically on mobile (desktop order unchanged). Made `AttendanceApp.tsx`'s root column a flex container and added mobile-only `order` rules (`.calendar-card { order: 1 }`, `.stat-cards-grid { order: 2 }`) inside the existing breakpoint — a pure CSS visual reorder, no JSX/component changes. See `ARCHITECTURE.md`'s "Mobile Responsiveness (Phase 15)" section.
...
**Status: Partially complete (testing done; production readiness started but not finished).** Verified 2026-09-05: all 12 E2E tests and all 19 component tests pass, re-run twice to rule out flakiness (one real test-authoring bug was caught and fixed this way — a `salary.spec.ts` test assumed the Salary Setup modal always opens blank, which broke on the second run once a prior test had already saved a rate for the test account). `npm run lint`/`npm run build` both pass clean (the generated `playwright/.cache/` build artifact had to be added to `eslint.config.mjs`'s ignore list and to `.gitignore`, alongside `playwright-report/`, `test-results/`, and `tests-ct/__snapshots__/`).

**Production readiness, started same day:** discussed with the owner how much security this small, single/two-user app actually needs — agreed the existing fundamentals (parameterized SQL, bcrypt, `httpOnly`/`sameSite`/`secure-in-prod` cookies, no public signup) were already proportionate, and that a WAF, CSP, or compliance tooling would be effort disproportionate to the risk. Two concrete, cheap gaps were identified and closed: (1) `lib/rateLimit.ts`, an in-memory per-IP limiter (5 failed attempts per 15-minute window, reset on success) wired into `app/api/auth/login/route.ts` — closes a real brute-force risk given the login page is reachable by anyone with the URL and the username is fixed/guessable; (2) baseline security response headers (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, `Strict-Transport-Security`) added via `next.config.ts`'s `headers()`. Verified directly against the running dev server: all five headers present on a real response, and a scripted run of 6 failed logins confirmed attempts 1–5 return 401 and the 6th returns 429. See `ARCHITECTURE.md`'s "Production Readiness (Phase 16)" section and `DECISIONS.md`.

**Production readiness, finished 2026-09-05:** closed the remaining gaps.
- **Env var audit**: confirmed `DATABASE_URL` is the only runtime env var the app reads anywhere (grepped for `process.env`), `.env.example` already documented it accurately, and no `NEXT_PUBLIC_*` variable exists to accidentally leak anything client-side. Added a fail-fast check in `lib/db.ts` — `DATABASE_URL` missing now throws a clear error at module load instead of surfacing as a cryptic `pg` connection failure later. Separately verified (reading `pg-connection-string`'s source, not just assuming) that `pg` already parses `sslmode` out of the connection string automatically — so Neon's SSL-required connection string (Phase 17) will work with zero code changes here.
- **Error-page polish**: added `app/not-found.tsx`, `app/error.tsx`, and `app/global-error.tsx`, styled to match the app's existing dark theme (reusing `lib/theme.ts` tokens and the `.btn-primary` class) instead of Next.js's default unstyled pages. Checked this Next.js version's actual `error.tsx`/`global-error.tsx` prop contract in `node_modules/next/dist/docs` before writing them — the `reset` prop from older Next.js versions was renamed `retry` in this one, exactly the kind of breaking change `AGENTS.md` warns about.
- **Two more items found and closed while auditing**: `package.json` gained an `engines.node` field (`>=20.9.0`, matching Next.js's own stated minimum) so Render provisions a compatible Node version; added `app/robots.ts` (disallow all crawlers) and `robots: {index:false, follow:false}` to `app/layout.tsx`'s metadata, since this is a private, login-gated personal tool with nothing worth search-engine exposure.
- **Real production-mode verification, not just build/lint**: ran `npm run build && npm run start` (this project's first `next start` test — everything before this had only ever been checked under `next dev`) and confirmed directly against that server: all five security headers present, the custom 404 renders with a real `404` status, `robots.txt` serves correctly, and a full login → authenticated request → logout → 401-after-logout cycle works with the session cookie correctly marked `Secure` (verified via curl's cookie jar, not just code inspection). Then ran the entire Playwright E2E suite against that real production server — all 12 pass, faster than under dev mode.

**Status: Complete.** Both halves of this phase's objective are done: real E2E/component test coverage (12 + 19 tests) and the production-readiness items above (security, env vars, error pages, Node version pinning, crawler exclusion), verified under both `next dev` and a real `next build && next start`. See `ARCHITECTURE.md`'s "Production Readiness (Phase 16)" section and `DECISIONS.md`.
...
**Status: Complete — live on Render's free tier.** Deploy-readiness (`render.yaml`, `/api/health`, the `engines.node` upper bound) was built and locally verified, then the owner explicitly authorized committing/pushing and performing the live deploy via Chrome browser automation. Pushed commit `9bf3aaa` to `origin/main`, then used Render's dashboard (owner already signed in; the owner supplied the Neon `DATABASE_URL` value into Render's prompted secret field directly — not typed by the assistant, per the credential-handling boundary) to create a Blueprint-managed web service (`track-me`, free plan, `oregon` region) from this repo's `render.yaml`. Two real deploy failures were hit and fixed before it went live — both are recorded in full in `DECISIONS.md`'s Phase 18 entry, in short: (1) `render.yaml`'s `NODE_ENV=production` reached the build step too, and npm's `NODE_ENV=production` → `--omit=dev` behavior silently skipped the `typescript`/`@types/*` devDependencies `next build`'s type-check needs — fixed by removing the var from `render.yaml`; (2) that fix alone didn't take effect on the next auto-deploy, because a plain `git push` triggers a code redeploy, not a full Blueprint sync that reconciles env vars against `render.yaml` — the stale `NODE_ENV` had to be deleted directly from the service's Environment tab before a rebuild picked up the fix. The resulting deploy succeeded (`Deploy succeeded | Live`, ~1 minute build), and was verified directly: `curl https://track-me-9fgr.onrender.com/api/health` → `200 {"status":"ok"}` with all five Phase 16 security headers present, and the root URL renders the real login page (dark theme, "Track Me" branding) in a real browser — confirming the app is genuinely serving traffic against the Neon database, not just that the build succeeded. Logging in with the real account was deliberately not attempted from this session (entering the owner's own password isn't something the assistant will do) — the owner should do that check themselves.
