...
- **Structure**: `app/layout.tsx` (minimal root HTML/body shell). `app/page.tsx` (the Phase 2 temporary backend test page — plain white background, no styling system) was removed in Phase 8 once `scripts/test-backend.mjs` could exercise the backend without it — see "Full Backend Testing & Temporary Page Removal (Phase 8)" below. No page component exists yet; `/` 404s until Phase 9 builds the real frontend.
