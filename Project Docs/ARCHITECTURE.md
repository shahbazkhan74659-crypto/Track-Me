...
- **Test harness**: `app/page.tsx`'s Date Entry section reuses the Calendar section's grid as the click target — clicking a current-month day opens an inline editor prefilled from any existing entry (status radios, an advance checkbox revealing an amount field, Save disabled until a status is chosen, Clear shown only when the day already has a saved entry).
...
- **Test harness**: `app/page.tsx`'s Summary section shows three plain cards (Earned So Far / Advance Taken / Net Payable) with present/half/leave subtext, refreshed on month navigation and — since the owner wants live recalculation "on every date entry" — also re-fetched immediately after `handleSaveEntry`, `handleClearEntry`, and `handleSaveSalary` succeed, so the cards update without a page reload.
- **Not yet built**: no frontend beyond the test harness (Phases 9–13); no currency/locale formatting (deferred to those phases, per the prototype's `fmtINR` helper being a display concern, not backend logic).
